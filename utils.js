...
/**
 * isBlank() - will check if a given string is blank; if string contains only whitespace(s) it will return false.
 * 
 * @param strng -
 ...
 * 
 * isBlank(undefined); - returns true
 * 
 */
function isBlank(strng) {
	...
	if (strng == null || strng == undefined || strng.trim() == "") {
		return true;
	}
	return false;
}
...
/**
 * isNotBlank() - will check if a given string is not blank; if string contains only whitespace(s) it will return true.
 * 
 * @param strng -
 *            String to be checked
 * 
 ...
 * 
 * isNotBlank(undefined); - returns false
 * 
 */
function isNotBlank(strng) {
	if (typeof strng !== "string" && !(strng instanceof String)) {
		return false;
	}
	return !isBlank(strng);
}

/**
 * isEmpty() - will check if a given string is empty; if string contains single whitespace it will return false.
 * 
 * @param strng -
 *            String to be checked
 * 
...
function isEmpty(strng) {
	...
	if (strng == null || strng == undefined || strng == "") {
		return true;
	}
	return false;
}
...
/**
 * isNotEmpty() - will check if a given string is not empty; if string contains single whitespace it will return true.
 * 
 * @param strng -
 *            String to be checked
 * 
...
function currencyFormatter(value, prefix, decimalPlaces, decimalSeparator, thousandSeparator, suffix) {
	try {
		...
		return prefix + s + (j ? i.substr(0, j) + thousandSeparator : "")
		        + i.substr(j).replace(/(\decimalSeparator{3})(?=\decimalSeparator)/g, "$1" + thousandSeparator)
